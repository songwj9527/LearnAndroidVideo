 ...
 *
 */
class SoulFlashVideoDrawer : IDrawer {
    /**上下颠倒的顶点矩阵*/
    ...
    private fun getFragmentShader(): String {
        ...
        return "#extension GL_OES_EGL_image_external : require\n" +
                ...
                "void main() {" +
                    ...
                    // 缩放比例[1.0,1.2]
                    "float scale = 1.0 + (1.2 - 1.0) * progress;" +
                    // 颜色偏移值范围[0,0.02]
                    "vec2 offsetCoords = vec2(0.02, 0.02) * progress;" +
                    ...
                    "vec4 flashMask = texture2D(uSoulTexture, soulTextureCoords);" +
                    // 原始颜色+offsetCoords
                    "vec4 maskR = texture2D(uSoulTexture, soulTextureCoords + offsetCoords);" +
                    // 原始颜色-offsetCoords
                    "vec4 maskB = texture2D(uSoulTexture, soulTextureCoords - offsetCoords);" +
                    // 原始纹理坐标下的纹素(颜色值rgba)
                    "vec4 mask = texture2D(uTexture, vCoordinate);" +

                    "if (drawFbo == 0) {" +
                    // 颜色混合 默认颜色混合方程式 = mask * (1.0-alpha) + weakMask * alpha
                    "    gl_FragColor = vec4(maskR.r, flashMask.g, maskB.b, flashMask.a);" +
                    "} else {" +

...
public class ModelLoaderRender implements GLSurfaceView.Renderer {
    private static final String TAG = "ModelLoaderRender";
...
//    };
    ...
    public ModelLoaderRender() {
        list = LoadObjectUtil.loadObject(rockDir + "/rock.obj",
                MyApplication.Companion.getInstance().getResources(), rockDir);
    }
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        textureFloor = TextureUtils.loadTextureNormal(MyApplication.Companion.getInstance(), R.drawable.ic_depth_testing_metal);
    }
    ...
    int generateProgram(){
        String vertexShaderStr = "attribute vec4 vPosition;" + // 顶点坐标
        ...
        String fragmentShaderStr = "precision mediump float;" +// 数据精度
                "varying vec2 aCoord;" +
                ...
                if (item != null) {
                    ...
                    if (item.mtl != null) {
                        if (!TextUtils.isEmpty(item.mtl.Kd_Texture)) {
                            if (item.diffuse < 0) {
                                try {
                                    Bitmap bitmap = BitmapFactory.decodeStream(MyApplication.Companion.getInstance().getAssets().open(
                            ...
                            if (item.diffuse < 0) {
                                item.diffuse = TextureUtils.loadTexture(MyApplication.Companion.getInstance(), R.drawable.ic_launcher_background);
                            }
                        }

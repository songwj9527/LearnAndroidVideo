...
import kotlinx.android.synthetic.main.activity_native_codec_player.*
import java.math.BigDecimal

class NativeCodecPlayerActivity : AppCompatActivity(), TextureView.SurfaceTextureListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_native_codec_player)

        filePath = intent.getStringExtra("file_path")

        nativePlayer = NativePlayer(NativePlayer.PlayerType.CODEC_PLAYER)
        Log.e(
                "NativeCodecActivity",
                "onCreate()"
        )
...
//                    handler?.sendEmptyMessage(1010)
        ...
        nativePlayer?.setOnErrorListener(object : NativePlayer.OnErrorListener {
            override fun onError(player: NativePlayer?, what: Int, extra: Int, msg: String?) {
                Log.e("NativeCodecActivity", "$what, $extra, $msg")
                nativePlayer?.reset()
                isStarted = false
                isPrepared = false
                isSeeking = false
        ...
        nativePlayer?.setOnPreparedListener(object : NativePlayer.OnPreparedListener {
            override fun onPrepared(player: NativePlayer?) {
                Log.e("NativeCodecActivity", "onPrepared()")
                isPrepared = true
//                nativePlayer?.start()
                try {
                    ...
                    Log.e("NativeCodecActivity", "video duration: $text")
                } catch (e: Exception) {
                    e.printStackTrace()
                }
//                nativePlayer?.setSurface(surface)
        ...
        nativePlayer?.setOnInfoListener(object : NativePlayer.OnInfoListener {
            override fun onInfo(
                ...
                texture_view = TextureRenderView(this@NativeCodecPlayerActivity)
                ...
                texture_view?.setSurfaceTextureListener(this@NativeCodecPlayerActivity)
        ...
        progress_seek_bar.setOnSeekBarChangeListener(object : SeekBar.OnSeekBarChangeListener {
            ...
            override fun onStartTrackingTouch(seekBar: SeekBar?) {
                Log.e("NativeCodecActivity", "onStartTrackingTouch()")
                if (isPrepared) {
                    nativePlayer?.pause()
                }
                seekBar?.let {
    ...
    override fun onPause() {
        Log.e("NativeCodecActivity", "onPause()")
        nativePlayer?.pause()
        super.onPause()
    }

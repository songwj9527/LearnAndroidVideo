...
FFRepack::FFRepack(JNIEnv *env, jstring in_path, jstring out_path) {
    jniEnv = env;
    jSrcPath = env->NewGlobalRef(in_path);
    jDesPath = env->NewGlobalRef(out_path);
    const char *src = env->GetStringUTFChars(in_path, NULL);
    const char *dest = env->GetStringUTFChars(out_path, NULL);
    ...
    if (srcPath == NULL || desPath == NULL) {
        if (jSrcPath != NULL && srcPath != NULL) {
            if (!env->IsSameObject(jSrcPath, NULL)) {
                env->ReleaseStringUTFChars((jstring) jSrcPath, desPath);
                env->DeleteGlobalRef(jSrcPath);
            }
        }
        if (jDesPath != NULL && desPath != NULL) {
            if (!env->IsSameObject(jDesPath, NULL)) {
                env->ReleaseStringUTFChars((jstring) jDesPath, desPath);
                env->DeleteGlobalRef(jDesPath);
            }
        }
        return;
    }
...
void FFRepack::Start() {
    ...
    // 关闭输入
    if (m_in_format_cxt != NULL) {
        avformat_close_input(&m_in_format_cxt);
        m_in_format_cxt = NULL;
    }
    LOGE(TAG, "Finish repacking, release resources 1")
    // 关闭输出
    if (m_out_format_cxt != NULL) {
        avio_close(m_out_format_cxt->pb);
        avformat_free_context(m_out_format_cxt);
        m_out_format_cxt = NULL;
    }
}
...
void FFRepack::Release() {
    ...
    LOGE(TAG, "Finish repacking, release resources 2")
    if (jSrcPath != NULL && srcPath != NULL) {
        if (!jniEnv->IsSameObject(jSrcPath, NULL)) {
            jniEnv->ReleaseStringUTFChars((jstring) jSrcPath, desPath);
            jniEnv->DeleteGlobalRef(jSrcPath);
        }
        jSrcPath = NULL;
        srcPath = NULL;
    }
    LOGE(TAG, "Finish repacking, release resources 3")
    if (jDesPath != NULL && desPath != NULL) {
        if (!jniEnv->IsSameObject(jDesPath, NULL)) {
            jniEnv->ReleaseStringUTFChars((jstring) jDesPath, desPath);
            jniEnv->DeleteGlobalRef(jDesPath);
        }
        jDesPath = NULL;
        desPath = NULL;
    }
    LOGE(TAG, "Finish repacking, release resources 4")
}